...
const char* ssid     = "FRITZ!Box 6590 Cable MJ";   //your network SSID
const char* password = "66129488961618186828";   //your network password

String myToken = "5880437840:AAFl6p_GYZbTLXZAXgMap45ud9CN69zlZMo";   // Create your bot and get the token -> https://telegram.me/fatherbot
String myChatId = "5016805884";        // Get chat_id -> https://telegram.me/userinfobot
...
void executeCommand(String text) {
  ...
  } else if (text=="/capture") {  //Ein aktuellen Screenshot vom Video bekommen
    sendCapturedImage2Telegram(myToken, myChatId);
  } else if (text=="/on") {  //FLASH-LED(Blitz) einschalten
    ledcAttachPin(4, 3);
    ledcSetup(3, 5000, 8);
    ledcWrite(3,10);
    sendMessage2Telegram(myToken, myChatId, "Turn on the flash", "");
  } else if (text=="/off") {  //FLASH-LED(Blitz) ausschalten
    ledcAttachPin(4, 3);
    ledcSetup(3, 5000, 8);
    ledcWrite(3,0);
    sendMessage2Telegram(myToken, myChatId, "Turn off the flash", "");
  } else if (text=="/restart") {  //Das komplette Modul(Gerät) neu starten
